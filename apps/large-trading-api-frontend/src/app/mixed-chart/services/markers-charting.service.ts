...
export class MarkersChartingService {
  ...
  public drawMarkers(
    ...
    if (this.getVisibleMarkersCount(chart) > threshold) {
      ...
      const allMarkers: SeriesMarker<Time>[] = [
        ...this.markersSignals,
        ...this.markersPivotPoints,
      ];
      allMarkers.sort(
        (a, b) => (a.time as UTCTimestamp) - (b.time as UTCTimestamp),
      );
  ...
  public highlightOpenSignals(
    ...
    this.markersSignals.forEach((marker: SeriesMarker<Time>) => {
      marker.size = newOpenTimes.has((marker.time as UTCTimestamp) * 1000)
        ? 3
        : undefined;
    });
  ...
  private setPivotPointMarkers(bars: Bar[]): void {
    ...
    bars.forEach((bar: Bar) => {
      ...
      compactMarkers.push({
        ...
        price:
          marker.position === 'belowBar' ? bar.prices.low : bar.prices.high,
    ...
    bars.forEach((bar: Bar) => {
      ...
      bar.chart.trendLines.forEach((trendLine: TrendLine) => {
        const startValue: number =
          trendLine.position === TrendLinePosition.Above
            ? bar.prices.high
            : bar.prices.low;
        ...
        if (trendLine.breakThroughIndex) {
          ...
          const lineFunction: LinearFunction = new LinearFunction(
            trendLine.function.m,
            trendLine.function.b,
          );
  ...
  private getVisibleMarkersCount(chart: IChartApi): number {
    ...
    const isInRange = (marker: SeriesMarker<Time>): boolean => {
      ...
      return (
        time >= (visibleRange!.from as UTCTimestamp) &&
        time <= (visibleRange!.to as UTCTimestamp)
      );
    };

    return (
      this.markersSignals.filter(isInRange).length +
      this.markersPivotPoints.filter(isInRange).length
    );
  }
  ...
  private getSignalTemplate(bar: Bar, strategy: Strategy): SeriesMarker<Time> {
    ...
    const hasForceClose: boolean =
      hasLiquidation || hasTakeProfit || hasStopLoss;
    const hasMultipleForceClose: boolean =
      [hasLiquidation, hasTakeProfit, hasStopLoss].filter((v) => v).length > 1;
    const isBuy: string | undefined =
      hasBuy && !hasSell && !hasClose && !hasForceClose ? 'BUY' : undefined;
    const isSell: string | undefined =
      hasSell && !hasBuy && !hasClose && !hasForceClose ? 'SELL' : undefined;
    const isClose: string | undefined =
      !hasBuy && !hasSell && !hasForceClose ? 'CLOSE' : undefined;
    const isLiquidation: string | undefined =
      hasLiquidation &&
      !hasMultipleForceClose &&
      !hasClose &&
      !hasBuy &&
      !hasSell
        ? 'LIQ'
        : undefined;
    const isTakeProfit: string | undefined =
      hasTakeProfit &&
      !hasMultipleForceClose &&
      !hasClose &&
      !hasBuy &&
      !hasSell
        ? 'TP'
        : undefined;
    const isStopLoss: string | undefined =
      hasStopLoss && !hasMultipleForceClose && !hasClose && !hasBuy && !hasSell
        ? 'SL'
        : undefined;
    const isCloseBuy: string | undefined =
      (hasClose || hasForceClose) &&
      !hasMultipleForceClose &&
      hasBuy &&
      !hasSell
        ? 'CBUY'
        : undefined;
    const isCloseSell: string | undefined =
      (hasClose || hasForceClose) &&
      !hasMultipleForceClose &&
      hasSell &&
      !hasBuy
        ? 'CSELL'
        : undefined;
    const isMix: string | undefined =
      (hasBuy && hasSell) ||
      (hasClose && hasForceClose) ||
      hasMultipleForceClose
        ? 'MIX'
        : undefined;
    const signal: string = (isBuy ||
      isSell ||
      isClose ||
      isLiquidation ||
      isTakeProfit ||
    ...
    const totalSize: number = backtestSignals.reduce(
      (acc: number, s: BacktestSignal) => {
    ...
    return {
      ...
      color: ['BUY', 'CBUY'].includes(signal)
        ? 'lime'
        : ['CLOSE', 'LIQ', 'TP', 'SL', 'MIX'].includes(signal)
          ? 'white'
          : '#ffd500',
  ...
  private getPivotPointTemplate(bar: Bar): SeriesMarker<Time> {
    ...
    return {
      time: (bar.times.open / 1000) as Time,
      position:
        pivotPoint.side === PivotPointSide.High ? 'aboveBar' : 'belowBar',
      color: 'white',
